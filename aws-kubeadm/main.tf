...
module "k8s" {
  ...
  worker_instances_spec = [for item in var.worker_instances_spec: ((tostring(lookup(item, "mayastor_node_label", "__missing__")) == "true") ? merge(item,{prefix="mayastor-worker"}) : (lookup(item, "prefix", "__missing__") == "__missing__" ? merge(item,{prefix="${substr(uuid(),0,var.num_chars_for_group_identifier)}", uuid_prefix = "true"}) : item))]
